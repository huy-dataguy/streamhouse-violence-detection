...
* **Warm Layer:** Raw telemetry and inference metadata ingestion via Spark Structured Streaming.
* **Aggregation Layer:** Aggregated analytical data (Star Schema) for trend analysis.
...
### 2. Lakehouse Incident Data Viewer (Raw Alerts)
![Live Command Center](assets/s-blob-v1-IMAGE-Bof979FWIkU.png)
A direct interface to the **Apache Iceberg Warm Layer**, allowing operators to monitor the raw data stream.
...
* **False Positive Management:** Operators can perform "Soft Deletes" on incorrect alerts, cleaning the dataset before it moves to the Aggregation Layer.
...
Visualizes data aggregated in the **Aggregation Layer** (Star Schema) for strategic decision-making.
